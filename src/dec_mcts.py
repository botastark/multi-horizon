...
class DecMCTSIntent:
    ...
    footprint_sequence: List[Tuple[int, int, int, int]] = field(default_factory=list)  # [(imin, imax, jmin, jmax), ...]
    ...
    def get_future_footprints(self, max_steps: int = None) -> List[Tuple[int, int, int, int]]:
...
class DecMCTSNode:
    ...
    def rollout(
        ...
        for t in range(max_depth):
            ...
            overlap_penalty = self.compute_overlap_penalty(state, imin, imax, jmin, jmax)
...
class DecMCTSPlanner:
    ...
    def plan(self) -> DecMCTSIntent:
        ...
        self._stats["avg_planning_time"] = (
            (self._stats["avg_planning_time"] * (n - 1) + planning_time) / n
        )
...
class DecMCTSCoordinator:
    ...
    def get_teammate_intents(self, agent_id: int) -> Dict[int, DecMCTSIntent]:
        ...
        with self._lock:
            return {
                aid: intent
                for aid, intent in self._intents.items()
                if aid != agent_id
            }
