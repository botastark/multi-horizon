...
from mcts import MCTSPlanner, copy_state
...
class IntentType(Enum):
    ...
    REGION_PRIORITY = "region_priority"       # HLP updated region priorities
    FRAGMENTATION_ALERT = "fragmentation_alert"  # HLP detected fragmentation
    
    # LLP -> HLP intents
    POSITION_UPDATE = "position_update"       # LLP reporting current position
    COVERAGE_UPDATE = "coverage_update"       # LLP reporting coverage changes
    ACTION_TAKEN = "action_taken"             # LLP executed an action
    REGION_REACHED = "region_reached"         # LLP reached target region
    
    # Bidirectional
    STATE_SYNC = "state_sync"                 # Full state synchronization
...
class IntentBus:
    ...
    def __init__(self, max_queue_size: int = 100):
        ...
        self._llp_to_hlp: queue.PriorityQueue = queue.PriorityQueue(maxsize=max_queue_size)
        self._hlp_to_llp: queue.PriorityQueue = queue.PriorityQueue(maxsize=max_queue_size)
    ...
    def shutdown(self):
        ...
        shutdown_intent = Intent(
            intent_type=IntentType.SHUTDOWN,
            source='bus',
            priority=999
        )
        try:
...
class LLPWorker(threading.Thread):
    ...
    def __init__(
        ...
        # Configuration
        self.short_horizon_depth = mcts_params.get('horizon_weights', {}).get('short_horizon_depth', 5)
    ...
    def request_action(self, state: Dict[str, Any], timeout: float = 5.0) -> Tuple[str, Dict]:
        """
    ...
    def _process_hlp_intents(self):
        ...
        while True:
            ...
            if intent.intent_type == IntentType.SHUTDOWN:
                ...
                logger.debug(f"LLP received target region: {intent.data.get('region_id')}")
    ...
    def _compute_action(self) -> Tuple[str, Dict]:
        ...
        if guidance is not None and time.time() < guidance.valid_until:
            ...
            metrics = {
                ...
                'guidance_age': time.time() - (guidance.valid_until - 5.0)  # Assuming 5s validity
            }
...
class HLPWorker(threading.Thread):
    ...
    def __init__(
        ...
        self.replan_interval = 1.0  # Seconds between replanning
        self.guidance_validity = 5.0  # Seconds guidance is valid
        
        # Hysteresis: current target gets small bonus to avoid ping-pong switching
        # But low enough that uncovered regions can still win
        self.current_target_bonus = 0.15  # Reduced: coverage bonus (0.8) should win for uncovered
    ...
    def run(self):
        ...
        while not self.intent_bus.is_shutdown():
            ...
            # Check for region reached event
            feedback = self.intent_bus.get_feedback()
            if feedback is not None and feedback.reached_target:
                should_replan = True
    ...
    def _process_llp_intents(self):
        ...
        while True:
            ...
            elif intent.intent_type == IntentType.COVERAGE_UPDATE:
                logger.debug(f"HLP: Coverage update received")
    ...
    def _compute_and_send_guidance(self):
        ...
        # Score regions
        # At high coverage, entropy (base_value) should dominate to guide toward uncertain areas
        # At low coverage, coverage_bonus helps prioritize uncovered regions
        overall_coverage = np.mean(covered_mask) if covered_mask is not None else 0.0
        
        region_scores = {}
        for region_id, metadata in region_metadata.items():
            # Entropy-based value (always relevant, even at 100% coverage)
            entropy_value = metadata['value']  # Higher for uncertain regions
            ...
            # At high coverage: let entropy dominate
            coverage_factor = 1.0 - overall_coverage  # 1.0 at 0% coverage, 0.0 at 100%
            coverage_bonus = (1.0 - metadata['coverage']) * self.w_coverage * coverage_factor
            
            isolation_score = isolation_scores.get(region_id, 0.0)
            isolation_bonus = isolation_score * self.w_fragmentation * 2.0
            
            # Combined score: entropy always matters, coverage bonus fades at high coverage
            # Entropy weight increases as coverage approaches 100%
            entropy_weight = 1.0 + overall_coverage  # 1.0 at 0%, 2.0 at 100%
            score = entropy_weight * entropy_value + coverage_bonus + isolation_bonus - 0.3 * distance_penalty
        ...
        if region_scores:
            ...
            if selected_region_id != self._current_target_region:
                logger.info(f"HLP: Switching target {self._current_target_region} -> {selected_region_id}")
                self._current_target_region = selected_region_id
        else:
            selected_region_id = 0
            selected_metadata = region_metadata.get(0, {'center': (H_dim/2, W_dim/2)})
        ...
        base_w_long = 0.4
        frag_boost = FRAGMENTATION_ADJUSTMENT_FACTOR * fragmentation_info['fragmentation_score']
        ...
        for i in range(n_rows):
            for j in range(n_cols):
                ...
                region_metadata[region_id] = {
                    ...
                    'entropy': entropy,
                    'coverage': coverage,
                    'value': entropy * (1.0 - coverage),
                    'uncovered_cells': int(np.sum(~region_covered))
                }
...
class ThreadedDualHorizonPlanner:
    ...
    def __init__(
        ...
        self.log_file = setup_dual_horizon_logger(experiment_name=experiment_name)
    ...
    def select_action(
        ...
        metrics['llp_step_count'] = self._llp_worker.step_count if self._llp_worker else 0
        metrics['hlp_replan_count'] = self._hlp_worker.replan_count if self._hlp_worker else 0
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        if self._llp_worker is not None:
            stats['llp'] = {
                ...
                'avg_planning_time': (
                    self._llp_worker.total_planning_time / max(self._llp_worker.step_count, 1)
                )
            }
        ...
        if self._hlp_worker is not None:
            stats['hlp'] = {
                ...
                'avg_planning_time': (
                    self._hlp_worker.total_planning_time / max(self._hlp_worker.replan_count, 1)
                )
            }
    ...
    def _log_step(
        ...
        if uav_pos is not None:
            log.info(f"UAV Position: ({uav_pos.position[0]:.2f}, {uav_pos.position[1]:.2f}, {uav_pos.altitude:.2f})")
        ...
        if guidance is not None:
            ...
            sorted_regions = sorted(guidance.region_scores.items(), key=lambda x: x[1], reverse=True)[:5]
        ...
        for act, score in sorted(blended_scores.items(), key=lambda x: x[1], reverse=True):
...
def create_threaded_planner(
    uav_camera: Any,
    conf_dict: Optional[Dict] = None,
    mcts_params: Optional[Dict] = None
) -> ThreadedDualHorizonPlanner:
    """
    Factory function to create a threaded dual-horizon planner.
    ...
    Returns:
        ThreadedDualHorizonPlanner instance
    """
    return ThreadedDualHorizonPlanner(
