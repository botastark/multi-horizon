...
def setup_dual_horizon_logger(
    log_dir: str = "logs", experiment_name: str = None
) -> str:
    """
    Set up a dedicated logger for dual-horizon planning analysis.
    Only initializes once; subsequent calls return the existing log file.
    ...
    # If already initialized, return existing log file
    if dual_horizon_log_file is not None:
    ...
    dual_horizon_logger.info(
        f"Experiment: {experiment_name if experiment_name else 'default'}"
    )
    dual_horizon_logger.info(f"Timestamp: {timestamp}")
    dual_horizon_logger.info("=" * 80)
    file_handler.flush()  # Ensure header is written immediately    return dual_horizon_log_file


def flush_dual_horizon_logger():
    """Flush the dual-horizon logger to ensure all logs are written to disk."""
    global dual_horizon_logger
    if dual_horizon_logger:
        for handler in dual_horizon_logger.handlers:
            handler.flush()
...
def log_planning_step(
    ...
    if uav_pos:
        dual_horizon_logger.info(
            f"UAV Position: {uav_pos.position}, Altitude: {uav_pos.altitude:.2f}"
        )
    ...
    if covered_mask is not None:
        ...
        dual_horizon_logger.info(
            f"Coverage Progress: {coverage:.4f} ({coverage*100:.2f}%)"
        )
    ...
    dual_horizon_logger.info(
        f"  Coverage Progress: {blend_weights.get('coverage_progress', 0):.4f}"
    )
    dual_horizon_logger.info(
        f"  Uncertainty Ratio: {blend_weights.get('uncertainty_ratio', 0):.4f}"
    )
    dual_horizon_logger.info(
        f"  Fragmentation Score: {blend_weights.get('fragmentation_score', 0):.4f}"
    )
    ...
    dual_horizon_logger.info(
        f"  Num Patches: {fragmentation_info.get('num_patches', 0)}"
    )
    dual_horizon_logger.info(
        f"  Total Uncovered: {fragmentation_info.get('total_uncovered', 0)}"
    )
    dual_horizon_logger.info(
        f"  Patch Sizes: {fragmentation_info.get('patch_sizes', [])[:10]}"
    )  # Show first 10
    dual_horizon_logger.info(
        f"  Fragmentation Score: {fragmentation_info.get('fragmentation_score', 0):.4f}"
    )
    ...
    dual_horizon_logger.info(
        f"  Action Scores: {json.dumps({k: f'{v:.4f}' for k, v in short_scores.items()}, indent=4)}"
    )
    ...
    dual_horizon_logger.info(
        f"  Action Scores: {json.dumps({k: f'{v:.4f}' for k, v in long_scores.items()}, indent=4)}"
    )
    ...
    dual_horizon_logger.info(
        f"Agreement: {'YES' if short_action == long_action == selected_action else 'PARTIAL' if selected_action in [short_action, long_action] else 'DIFFERENT'}"
    )
...
def log_summary_statistics(
    total_steps: int, final_coverage: float, additional_stats: Optional[Dict] = None
):
    """
    ...
    dual_horizon_logger.info(
        f"Final Coverage: {final_coverage:.4f} ({final_coverage*100:.2f}%)"
    )
...
def compute_long_horizon_reward(
    ...
    revisit_cost = compute_revisit_cost(current_pos, fragmentation_info, uav_speed=1.0)
...
class DualHorizonPlanner:
    ...
    def finalize_episode(self, final_state: Dict[str, Any]):
        ...
        short_pct = (
            (self.short_selected_count / total_decisions * 100)
            if total_decisions > 0
            else 0
        )
        long_pct = (
            (self.long_selected_count / total_decisions * 100)
            if total_decisions > 0
            else 0
        )
    ...
    def short_horizon_plan(self, state: Dict[str, Any]) -> Tuple[str, Dict[str, Any]]:
        """
    ...
    def long_horizon_plan(self, state: Dict[str, Any]) -> Tuple[int, Dict[str, Any]]:
        """
        ...
        region_map, region_metadata = partition_field(
            belief, covered_mask, self.tile_size
        )
        ...
        current_row, current_col = self.camera.convert_xy_ij(
            uav_pos.position[0], uav_pos.position[1], self.camera.grid.center  # x  # y
        )
        ...
        #
        # This elegantly balances coverage and confidence:
        ...
        # Configurable weights for the unified formula
        exploration_bonus_weight = (
            self.w_coverage * 2.0
        )  # β: how much to boost uncovered

        for region_id, metadata in region_metadata.items():
            # Primary metric: average entropy (uncertainty to reduce)
            region_entropy = metadata.get("entropy", 0.0)
            num_cells = max(1, metadata.get("num_cells", 1))
            avg_entropy = region_entropy / num_cells
            ...
            # UNIFIED FORMULA:
            # Score = Entropy × (1 + β(1-Coverage)) - γ × Distance
            #
            # - When coverage=0: Score = Entropy × (1 + β) - γd  (exploration bonus)
            # - When coverage=1: Score = Entropy × 1 - γd        (pure entropy)
            # - High entropy always wins over low entropy, regardless of coverage
            exploration_bonus = 1.0 + exploration_bonus_weight * (1.0 - coverage)

            # Isolation bonus for fragmented uncovered patches
            isolation_score = isolation_scores.get(region_id, 0.0)
            isolation_bonus = isolation_score * self.w_fragmentation
        ...
        if region_scores:
            selected_region_id = max(
                region_scores.keys(), key=lambda r: region_scores[r]
            )
        ...
        for rid in sorted(
            region_scores.keys(), key=lambda r: region_scores[r], reverse=True
        )[:5]:
        ...
        print(
            f"[HLP] Fragmentation - patches: {fragmentation_info['num_patches']}, "
    ...
    def _identify_regions_with_isolated_patches(
        ...
        for patch_id in range(1, num_patches + 1):
            ...
            if regions_in_patch:
                best_region = max(
                    regions_in_patch.keys(), key=lambda r: regions_in_patch[r]
                )
    ...
    def _compute_region_fragmentation_impact(
        ...
        neighborhood = covered_mask[
            neighbor_row_min:neighbor_row_max, neighbor_col_min:neighbor_col_max
        ]
    ...
    def dual_horizon_plan(self, state: Dict[str, Any]) -> Tuple[str, Dict[str, Any]]:
        """
        ...
        if self.cached_target_region_id is None:
            ...
            if covered_mask is not None and self.cached_region_metadata is not None:
                target_metadata = self.cached_region_metadata.get(
                    self.cached_target_region_id, {}
                )
                bounds = target_metadata.get("bounds")

                if bounds is not None:
                    (row_min, row_max), (col_min, col_max) = bounds
                    region_coverage_mask = covered_mask[
        ...
        if need_hlp_replan:
            ...
            self.steps_since_hlp_replan = 0
            print(
                f"[HLP] REPLANNING - Reason: {replan_reason} → New target region: {target_region_id}"
            )
        else:
            ...
            print(
                f"[HLP] REUSING cached target region: {target_region_id} (steps since replan: {self.steps_since_hlp_replan})"
            )
        ...
        print(
            f"  Current UAV (spatial): ({uav_pos.position[0]:.2f}, {uav_pos.position[1]:.2f}, alt={uav_pos.altitude:.2f})"
        )
        print(
            f"  Distance to target: {np.linalg.norm(current_pos - target_pos):.2f} grid cells"
        )
        print(
            f"  Weights: w_short={blend_weights['w_short']:.3f}, w_long={blend_weights['w_long']:.3f}"
        )
        ...
        for action in all_actions:
            ...
            combined_val = (
                blend_weights["w_short"] * ig_val + blend_weights["w_long"] * align_val
            )
            ...
            if action in ["up", "down"]:
                print(
                    f"  {action:6s}: IG={ig_val:.4f} × {blend_weights['w_short']:.3f} = {combined_val:.4f} [altitude: IG only]"
                )
            else:
                print(
                    f"  {action:6s}: IG={ig_val:.4f} × {blend_weights['w_short']:.3f} + Align={align_val:.4f} (raw={raw_align:.1f}) × {blend_weights['w_long']:.3f} = {combined_val:.4f}"
                )

        # Select action with highest combined score
        if combined_scores:
            selected_action = max(
                combined_scores.keys(), key=lambda a: combined_scores[a]
            )
        else:
        ...
        best_aligned_action = (
            max(alignment_scores.keys(), key=lambda a: alignment_scores[a])
            if alignment_scores
            else None
        )
        agreement_type = (
            "YES"
            if selected_action == short_action == best_aligned_action
            else (
                "PARTIAL"
                if selected_action in [short_action, best_aligned_action]
                else "DIFFERENT"
            )
        )
        ...
        print(
            f"  Blend weights: w_short={blend_weights['w_short']:.3f}, w_long={blend_weights['w_long']:.3f}"
        )
        ...
        flush_dual_horizon_logger()

        return selected_action, metrics

    def _compute_blend_weights(self, state: Dict[str, Any]) -> Dict[str, float]:
        """
...
def create_dual_horizon_planner(
    uav_camera: Any, conf_dict: Optional[Dict] = None, config: Optional[Dict] = None
) -> DualHorizonPlanner:
    """
