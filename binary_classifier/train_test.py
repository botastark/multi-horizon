...
def train(audio_model, train_loader, test_loader, args):
    task = Task.init(project_name="binary_classifier", task_name="train")
    ...
    optimizer = torch.optim.AdamW(trainables, lr=args.lr, weight_decay=0.3)
    ...
    def get_len(trainset=True):
        ...
        if trainset:
            drop_last_ = n_train % batch_size
            len_dataset = int((n_train - drop_last_) / batch_size)
        else:
            drop_last_ = n_valid % batch_size
            len_dataset = int((n_valid - drop_last_) / batch_size)
        if not drop_last and drop_last_ > 0:
            len_dataset += 1
        return len_dataset
    ...
    print(
        "now training main metrics: {:s}, loss function: {:s}, learning rate scheduler: {:s}".format(
            str(main_metrics), str(loss_fn), str(scheduler)
        )
    )
    print(
    ...
    audio_model.train()
    dataset_len = get_len(trainset=True)
    if dataset_len == None:
    ...
    while epoch < args.n_epochs + 1:
        ...
        for i, (audio_input, labels) in enumerate(train_loader):
            ...
            if print_step and global_step != 0:
                ...
                Logger.current_logger().report_scalar(
                    "train",
                    "train_loss",
                    iteration=((epoch - 1) * dataset_len + i),
                    value=loss_meter.item(),
                )
        ...
        #     torch.save(optimizer.state_dict(), "%s/models/optim_state.%d.pth" % (exp_dir, epoch))
        Logger.current_logger().report_scalar(
            "valid", "avg_train_loss", iteration=epoch, value=loss_meter.avg
        )
        Logger.current_logger().report_scalar(
            "valid", "valid_loss", iteration=epoch, value=valid_loss
        )

        Logger.current_logger().report_scalar(
            "test", "valid_loss", iteration=epoch, value=valid_loss
        )
        Logger.current_logger().report_scalar(
            "test", "accuracy", iteration=epoch, value=acc * 100
        )

        Logger.current_logger().report_scalar(
            "valid", "accuracy", iteration=epoch, value=acc * 100
        )
        scheduler.step()
...
def validate(audio_model, val_loader, args, epoch):
    ...
    with torch.no_grad():
        ...
        print("saved predoctions")
        print(exp_dir + "/predictions/predictions_" + str(epoch) + ".csv")
